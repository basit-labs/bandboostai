...
export default function ReadingPage() {
  ...
  const startSession = async () => {
    try {
      const res = await apiRequest("GET", "/api/practice/session/reading");
  ...
  const submitMutation = useMutation({
    mutationFn: async () => {
      ...
      await apiRequest("POST", "/api/tasks/complete-by-type", { taskType: "reading" });
      return results;
    },
    onSuccess: () => {
      setSubmitted(true);
        ...
        <motion.div className="space-y-6 pb-20 md:pb-0 max-w-4xl" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          <div className="flex items-center justify-between">
            <Button
              variant="ghost"
              size="sm"
              onClick={() => { setSessionStarted(false); setSessionData(null); setAnswers({}); setSubmitted(false); }}
          ...
          <div className="space-y-4">
            {questions.map((q: any, index: number) => {
              ...
              return (
                <Card key={q.id} className={submitted ? (isCorrect ? "border-chart-2/30" : answers[q.id] ? "border-destructive/30" : "") : ""} data-testid={`card-rq-${index}`}>
                  <CardContent className="p-5">
                      ...
                      {options.map((opt: string, oi: number) => (
                        <button
                          ...
                          className={`w-full text-left p-3 rounded-lg border text-sm transition-colors ${
                            answers[q.id] === opt
                              ? submitted ? (isCorrect ? "border-chart-2 bg-chart-2/10" : "border-destructive bg-destructive/10") : "border-primary bg-primary/5"
                              : submitted && opt === q.correctAnswer ? "border-chart-2 bg-chart-2/10" : "hover:bg-muted/50"
                          }`}
                          onClick={() => { if (!submitted) setAnswers({ ...answers, [q.id]: opt }); }}
                          disabled={submitted}
                          data-testid={`option-rq-${q.id}-${oi}`}
                        >
                          <div className="flex items-center gap-2">
                    ...
                    {submitted && q.explanation && (
                      <p className="text-xs text-muted-foreground mt-3 p-2 bg-muted/50 rounded">{q.explanation}</p>
                    )}
                  </CardContent>
                </Card>
              );
          ...
          {submitted && (
            <Card className="bg-gradient-to-r from-chart-2/10 to-transparent border-chart-2/20">
              <CardContent className="p-6 text-center">
                ...
                <h3 className="text-xl font-bold" data-testid="text-reading-score">
                  {correctCount} / {totalQuestions} Correct
                </h3>
                <p className="text-sm text-muted-foreground mt-1">Score: {Math.round((correctCount / totalQuestions) * 100)}%</p>
                <Button className="mt-4" onClick={() => { setSessionStarted(false); setSessionData(null); setAnswers({}); setSubmitted(false); }} data-testid="button-new-session">
                  Start New Session
                </Button>
              </CardContent>
            </Card>
  ...
  const dailyUsed = usageData?.usage?.readingCount || 0;
  ...
  return (
    <AppShell>
      <motion.div className="space-y-6 pb-20 md:pb-0" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2" data-testid="text-reading-page-title">
            <BookOpen className="w-6 h-6 text-chart-2" />
            Reading Practice
          </h1>
          <p className="text-muted-foreground text-sm mt-1">Session-based reading comprehension with smart question selection</p>
        </div>
        ...
        <Card className="border-chart-2/20">
          <CardContent className="p-8 text-center">
            ...
            <h2 className="text-xl font-bold mb-2">Reading Session</h2>
            ...
            {!isPremium && dailyUsed >= (dailyLimit || 3) ? (
              <div className="space-y-3">
                ...
                <Button variant="outline" className="border-chart-4/30 text-chart-4" data-testid="button-upgrade-reading">
                  <Crown className="w-4 h-4 mr-2" />
                  Upgrade to Premium
                </Button>
              </div>
            ) : (
              <Button size="lg" onClick={startSession} data-testid="button-start-reading-session">
